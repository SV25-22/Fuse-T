...
def build_text_example(thread: dict, tokenizer, k_replies: int) -> str:
    ...
    return f" {sep} ".join([root_text] + replies)
...
class FusionGraphDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        threads: List[dict],
        tokenizer,
        k_replies: int,
        undirected: bool,
        norm: Optional[Tuple[torch.Tensor, torch.Tensor]],
        gfeat_norm: Optional[Tuple[torch.Tensor, torch.Tensor]],
        early_minutes: int,
        early_k: int,
    ):
    ...
    def __getitem__(self, idx):
        ...
        data = build_graph_from_thread(
            t,
            make_undirected=self.undirected,
...
class GraphEncoder(nn.Module):
    ...
    def forward(self, data) -> torch.Tensor:
        ...
        if self.readout == "mean":
            ...
            ptr = data.ptr
            root_global = ptr[:-1] + data.root_idx
            g = x[root_global]
        else:
            raise ValueError("readout must be mean|meanmax|root")

        if hasattr(data, "gfeat"):
            gf = data.gfeat
            if gf.dim() == 1:
                gf = gf.view(g.size(0), -1)
            elif gf.dim() == 2 and gf.size(0) != g.size(0):
                gf = gf.view(g.size(0), -1)
...
class TextModel(nn.Module):
    def __init__(self, model_name: str, dropout: float, num_classes: int = 2):
        super().__init__()
...
class ResidualInjectionFusion(nn.Module):
    def __init__(self, text_dim: int, graph_dim: int, dropout: float,
                 use_gate: bool = True, gate_bias_init: float = -4.0, gate_weight_scale: float = 0.01):
        super().__init__()
        self.use_gate = use_gate

        self.proj_g = nn.Sequential(
            nn.LayerNorm(graph_dim),
            nn.Linear(graph_dim, text_dim),
            nn.GELU(),
            nn.Dropout(dropout)
        )
        ...
        if self.use_gate:
            ...
            nn.init.xavier_uniform_(self.gate.weight, gain=gate_weight_scale)
            nn.init.constant_(self.gate.bias, gate_bias_init)
        else:
            self.gate = None

    def forward(self, h_text: torch.Tensor, h_graph: torch.Tensor):
        hg_proj = self.proj_g(h_graph)

        if self.gate is None:
            h_fused = h_text + hg_proj
        else:
            alpha = torch.sigmoid(self.gate(torch.cat([h_text, hg_proj], dim=-1)))
            h_fused = h_text + alpha * hg_proj
...
class ConcatFusion(nn.Module):
    ...
    def forward(self, h_text: torch.Tensor, h_graph: torch.Tensor):
        hg_proj = self.proj_g(h_graph)
        return self.mix(torch.cat([h_text, hg_proj], dim=-1))


def get_text_list(batch) -> List[str]:
    t = batch.text
    if isinstance(t, str):
        return [t]
    if isinstance(t, (list, tuple)):
        return list(t)
    return [str(x) for x in list(t)]

class GraphHead(nn.Module):
    def __init__(self, graph_dim: int, dropout: float, num_classes: int = 2):
        super().__init__()
...
@torch.no_grad()
def evaluate(fusion, gnn_enc, g_head, txt, loader, tokenizer, max_length, device):
    fusion.eval(); gnn_enc.eval(); g_head.eval(); txt.eval()

    ys, pf, pt, pg = [], [], [], []

    for batch in loader:
        batch = batch.to(device)
        texts = get_text_list(batch)
        ...
        h_text, logits_text = txt(enc)

        h_fused = fusion(h_text, h_graph)
        logits_fused = txt.fc(h_fused)

        ys.extend(y.cpu().tolist())
        pf.extend(logits_fused.argmax(-1).cpu().tolist())
        pt.extend(logits_text.argmax(-1).cpu().tolist())
        pg.extend(logits_graph.argmax(-1).cpu().tolist())

    out = {
        "f1_fused": f1_score(ys, pf, average="macro"),
        "f1_text":  f1_score(ys, pt, average="macro"),
        "f1_graph": f1_score(ys, pg, average="macro"),
        "acc_fused": accuracy_score(ys, pf),
    }
    print(out)
    
    return out["f1_fused"], out["acc_fused"]
...
def main():
    ...
    ap.add_argument("--k_replies", type=int, default=0)
    ...
    ap.add_argument(
        ...
        choices=["residual_gate", "residual_nogate", "concat"],
    )
    ap.add_argument("--gate_bias_init", type=float, default=-4.0)
    ap.add_argument("--gate_weight_scale", type=float, default=0.01)
    ...
    ap.add_argument("--lr_graph", type=float, default=1e-3)
    ap.add_argument("--lr_fuse", type=float, default=1e-3)
    ap.add_argument("--freeze_text_epochs", type=int, default=1)
    ap.add_argument("--gnn_warmup_epochs", type=int, default=3)
    ap.add_argument("--init_text_ckpt", type=str, default="")
    ap.add_argument("--init_gnn_ckpt", type=str, default="")
    ...
    gmean, gstd = fit_gfeat_normalizer(
        train_threads,
        undirected=args.undirected,
        early_minutes=args.early_minutes,
        early_k=args.early_k,
    ...
    train_ds = FusionGraphDataset(
        train_threads, tokenizer, args.k_replies, args.undirected,
        (mean, std), (gmean, gstd), args.early_minutes, args.early_k,
    )
    val_ds = FusionGraphDataset(
        val_threads, tokenizer, args.k_replies, args.undirected,
        (mean, std), (gmean, gstd), args.early_minutes, args.early_k,
    )
    test_ds = FusionGraphDataset(
        test_threads, tokenizer, args.k_replies, args.undirected,
        (mean, std), (gmean, gstd), args.early_minutes, args.early_k,
    )
    ...
    txt = TextModel(args.model_name, dropout=args.dropout, num_classes=2).to(device)

    ro = args.hidden_dim if args.readout != "meanmax" else args.hidden_dim * 2
    gfeat_dim = train_ds[0].gfeat.size(-1)
    graph_out_dim = ro + gfeat_dim
    gnn_enc = GraphEncoder(7, args.hidden_dim, args.num_layers, args.dropout, args.readout, gfeat_dim=gfeat_dim).to(device)
    g_head = GraphHead(graph_dim=graph_out_dim, dropout=args.dropout, num_classes=2).to(device)

    if args.init_text_ckpt:
        ck = torch.load(args.init_text_ckpt, map_location="cpu")
        sd = ck.get("state_dict", ck)
        missing, unexpected = txt.load_state_dict(sd, strict=False)
        print("Loaded text model from", args.init_text_ckpt)

    if args.init_gnn_ckpt:
        ck = torch.load(args.init_gnn_ckpt, map_location="cpu")
        sd = ck.get("state_dict", ck)
        sd = {k: v for k, v in sd.items() if k.startswith("convs.")}
        gnn_enc.load_state_dict(sd, strict=False)
        print("Loaded graph convs from", args.init_gnn_ckpt)


    if args.fusion_type == "concat":
        fusion = ConcatFusion(text_dim=txt.hidden, graph_dim=graph_out_dim, dropout=args.dropout).to(device)
    elif args.fusion_type == "residual_nogate":
        fusion = ResidualInjectionFusion(
            text_dim=txt.hidden, graph_dim=graph_out_dim, dropout=args.dropout,
            use_gate=False
        ).to(device)
    else:
        fusion = ResidualInjectionFusion(
            text_dim=txt.hidden, graph_dim=graph_out_dim, dropout=args.dropout,
            use_gate=True, gate_bias_init=args.gate_bias_init, gate_weight_scale=args.gate_weight_scale
        ).to(device)

    params = (
        list(txt.parameters())
        + list(gnn_enc.parameters())
        + list(g_head.parameters())
        + list(fusion.parameters())
    )
    
    opt = AdamW(
        [
            {"params": txt.parameters(), "lr": args.lr_text},
            {"params": list(gnn_enc.parameters()) + list(g_head.parameters()), "lr": args.lr_graph},
            {"params": fusion.parameters(), "lr": args.lr_fuse},
        ],
    ...
    for epoch in range(1, args.epochs + 1):
        gnn_warmup_phase = epoch <= args.gnn_warmup_epochs
        text_frozen = epoch <= args.freeze_text_epochs

        for p in txt.parameters():
            p.requires_grad = not text_frozen
        for p in fusion.parameters():
            p.requires_grad = not gnn_warmup_phase
        ...
        for batch in tqdm(train_loader, desc=f"train epoch {epoch} (GNN Warmup: {gnn_warmup_phase})", leave=False):
            batch = batch.to(device)
            ...
            h_graph = gnn_enc(batch)
            logits_graph = g_head(h_graph)
            loss_g = F.cross_entropy(logits_graph, y, weight=class_w)
            ...
            if gnn_warmup_phase:
                ...
                loss_f = F.cross_entropy(logits_fused, y, weight=class_w)
                loss = loss_f + args.aux_weight * loss_g
        ...
        val_f1, val_acc = evaluate(fusion, gnn_enc, g_head, txt, val_loader, tokenizer, args.max_length, device)

        print(
            f"[epoch {epoch:02d}] loss={total_loss/max(1,len(train_loader)):.4f} val_f1={val_f1:.4f} val_acc={val_acc:.4f}"
        )

        if not gnn_warmup_phase and val_f1 > best_val:
            best_val = val_f1
            best_state = {
                "txt": {k: v.detach().cpu() for k, v in txt.state_dict().items()},
                "gnn_enc": {k: v.detach().cpu() for k, v in gnn_enc.state_dict().items()},
                "g_head": {k: v.detach().cpu() for k, v in g_head.state_dict().items()},
                "fusion": {k: v.detach().cpu() for k, v in fusion.state_dict().items()},
            }
            bad = 0
        elif not gnn_warmup_phase:
            bad += 1
    ...
    txt.load_state_dict(best_state["txt"])
    ...
    test_f1, test_acc = evaluate(fusion, gnn_enc, g_head, txt, test_loader, tokenizer, args.max_length, device)

    result = {
        "title": "Fuse-T Residual Modality Injection",
        "fold": args.fold,
        "seed": args.seed,
        ...
        "gate_weight_scale": float(args.gate_weight_scale),
    }

    (out_dir / "result.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
    torch.save(
        {"best_state": best_state, "mean": mean, "std": std, "config": vars(args)},
        out_dir / "best.pt",
    )
